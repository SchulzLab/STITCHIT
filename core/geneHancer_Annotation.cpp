...
int main(int argc, char *argv[]){
	...
	desc.add_options()
		...
		("geneHancerFile,k",boost::program_options::value<std::string>(&peakFile),"File containing the annoted enhancers in bed file format <chr> <start> <end> <ENSGID>. The file must be sorted according to the geneIDs.")
	...
	if (not(vm.count("geneHancerFile"))){
	...
	pr.findEnhancers(geneID);
	std::vector<std::tuple<std::string, unsigned int, unsigned int> > genomeConv= pr.getEnhancers();
	std::cout<<"Identified "<<genomeConv.size()<<" enhancers."<<std::endl;

	if (genomeConv.size()==0){
		std::cout<<"No regions found. Aborting."<<std::endl;
		return 0;
	}
	...
	bs.computeMeanSignal(genomeConv);
	...
	if (corM=="Both"){
		...
		for (unsigned int i=0; i<genomeConv.size();i++){
			std::cout<<std::get<1>(genomeConv[i])<<"	"<<std::get<2>(genomeConv[i])<<"	"<<corP[i].first<<"	"<<corP[i].second<<"	"<<corS[i].first<<"	"<<corS[i].second<<std::endl;
		}
